...
public class Store {
	...
	public static int iconSpaceY = 15;
	public static int itemIn = 4;
	public static int heldID = -1;
	public static int realID = -1;

	public static int[] buttonID = { Value.airTowerLaser, Value.airTowerFreeze,
			Value.airTowerBlaster, Value.airTowerStopper,
			Value.airTowerMoneyTree, Value.airTowerTurret, Value.airAir,
			Value.airTrash };
	public static int[] buttonPrice = { 10, 50, 100, 150, 125, 70, 0, 0 };
	...
	public void define() {
		for (int i = 0; i < button.length; i++) {
			button[i] = new Rectangle((Screen.myWidth / 2)
					...
					(Screen.room.block[Screen.room.getWorldHeight() - 1][0].y)
							+ Screen.room.getBlockSize() + awayFromRoom,
	...
	public void draw(Graphics g) {
		...
		g.drawString("" + Screen.health, buttonHealth.x + buttonHealth.width
				+ iconSpace, buttonHealth.y + iconSpaceY);

		g.drawString("" + Screen.coinage, buttonCoins.x + buttonCoins.width
				+ iconSpace, buttonCoins.y + iconSpaceY);
